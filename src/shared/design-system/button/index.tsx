...
const Button = ({ className = '', color = 'primary', ...otherProps }: TButtonProps) => {
  if (color === 'primary') {
    ...
    return (
      <button
        className={`text-white hover:text-teal-300 ring-1 ring-white hover:ring-teal-300 focus:ring-4 focus:ring-teal-300 font-medium rounded-lg text-sm px-5 py-2.5 focus:outline-none ${className}`}
        {...otherProps}
      />
    );
  }
